...
function initSearch(search, contentMain, searchMenu) {
    return __awaiter(this, void 0, void 0, function () {
        var searchBar, options_1, timeout_1;
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.sent();
                    evaluateSearch(search.get("search"), options_1);
                    return [3, 4];
    ...
    try {
        for (var data_1 = __values(data), data_1_1 = data_1.next(); !data_1_1.done; data_1_1 = data_1.next()) {
            ...
            if (!options.properties.checked && (res.type === 6 || res.type === 7))
                continue;
            if (!options.methods.checked && res.type === 8)
                continue;
                            ...
                            options.properties.onchange =
                                options.methods.onchange =
                                    options.enumMembers.onchange =
                                        options.thisModuleOnly.onchange = function () { return searchResults && displayResults(filterResults(options, searchResults)); };
    return options;
}
function formatResult(res) {
    ...
    switch (res.type) {
        ...
        case 8: {
            content = "<div>\n            <a href=\"" + window.depth + path.map(function (m) { return "m." + m; }).join("/") + "/class/" + res.obj + ".html#." + res.name + "\"><span class=\"item-name object\">" + res.obj + "</span><span class=\"symbol\">.</span><span class=\"item-name method-name\">" + res.highlighted + "</span></a>\n            " + (path.length ? "<p class=\"docblock secondary\">In " + path.join("/") + "</p>" : "") + "\n            </div>";
            break;
        }
        case 7: {
                    ...
                    try {
                        for (_a = __values(data[0]), _b = _a.next(); !_b.done; _b = _a.next()) {
                            ...
                            searchData.push.apply(searchData, __spreadArray([], __read(module_1[1].map(function (cl) {
                                ...
                                searchData.push.apply(searchData, __spreadArray([], __read(cl[1].map(function (p) { return ({ name: p, path: path, obj: cl[0], type: 6 }); }))));
                                searchData.push.apply(searchData, __spreadArray([], __read(cl[2].map(function (p) { return ({ name: p, path: path, obj: cl[0], type: 8 }); }))));
                                return {
                                    name: cl[0],
                                    path: path,
                                    type: 0
